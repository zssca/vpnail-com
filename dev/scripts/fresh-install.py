...
def find_project_root() -> Path:
    """Locate project root by finding the nearest package.json"""
    script_path = Path(__file__).resolve()
    for parent in script_path.parents:
        if (parent / "package.json").exists():
            return parent

    print_error("Could not locate project root (package.json not found)")
...
def main():
    ...
    # Locate project root by walking up to nearest package.json
