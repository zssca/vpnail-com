...
export function NavigationButton({ direction, canNavigate, onNavigate }: NavigationButtonProps) {
  ...
  return (
    <div
      className={`absolute ${isPrevious ? 'left-0' : 'right-0'} top-1/2 -translate-y-1/2 transform p-4 text-muted-foreground/60 transition-colors ${
        canNavigate ? 'cursor-pointer hover:text-foreground' : 'cursor-not-allowed opacity-25'
      }`}
      onClick={onNavigate}
      onKeyDown={(e) => {
...
export function LightboxNavigation({
  ...
  return (
    <>
      {/* Image counter badge */}
      <div className="absolute right-4 top-4 rounded bg-background/80 px-3 py-1.5 text-sm text-foreground backdrop-blur-sm">
